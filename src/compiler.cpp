...
compiler::compiler(cmp_args& args)
{
    switch (args.bits)
    {
        ...
        case 16:
            ...
            break;
        default:
            ERR(err_t::CMP_BITS_UNSUPP, args.bits);
            break;
    }
    this->args = &args;
}
...
void compiler::post_as(std::string& i_file, std::string& o_file)
{
    ...
    nasm << "nasm -f elf" << __BITS__ << " " << i_file << " -o " << o_file;
...
cmp_args::cmp_args(size_t bits, std::vector<std::string> inputs, std::string output)
    : bits(bits), inputs(inputs), output(output)
{
    if (bits == -1)
