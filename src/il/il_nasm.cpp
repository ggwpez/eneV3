...
il_nasm::il_nasm(ProgramNode* code, std::wostringstream* stream)
	: il(code, stream)
{
...
void il_nasm::generate(PushNode* code)
{
	...
	else if (VariableNode* v = dynamic_cast<VariableNode*>(code->v))     //its a variable in the stack frame
	{
		if (v->ebp_off)
		{
			...
			eml(L"lea " << rdx << L", [ebp " << adder << L"]\t; push " << v->var_name->str);
			PUSH(rdx);
		}
		else
		{
...
void il_nasm::generate_op_drf(OperatorNode* code)
{
	...
	{
		case 1:
			eml(L"mov  " <<  al << L", byte [" << rcx << L"]");
			break;
		case 2:
			eml(L"mov  " <<  ax << L", word [" << rcx << L"]");
			break;
		case 4:
			eml(L"mov  " << eax << L", dword [" << rcx << L"]");
			break;
		case 8:
			eml(L"mov  " << rax << L", qword [" << rcx << L"]");
			break;
		default:
			ERR(err_t::GEN_IL);
			break;
...
{
	eml(L"jmp " << il::__str_break << std::to_wstring(brk_c));
};
...
void il_nasm::generate(ForNode* code)
{
	...
	generate(code->inc);                //i++
	eml(L"jmp " << check_name);          //start over
...
void il_nasm::initalize_streams()
{
	emlBSS(L"section .bss");
	emlCODEH(L"section .text");
	emlDATA(L"section .data");
}

void il_nasm::finalize_streams()
{
	*this->ss << ss_codeh->str().c_str() << std::endl <<
